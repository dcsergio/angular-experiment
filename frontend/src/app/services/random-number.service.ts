...
export class RandomNumberService {
  ...
  constructor(private readonly http: HttpClient) { this.startTimestamp = Date.now(); }

  getRandomNumber$(): Observable<string> {
    return timer(0, this.intervalMs).pipe(
      // 1. switchMap: moves from the timer Observable to HTTP Observablle
      switchMap(() => {
        ...
        return this.http.get(this.apiUrl, { responseType: 'text' }).pipe(
          ...
          map((response) => {
            ...
            let s: string = n.toString() + '@' + Math.round((Date.now() - this.startTimestamp) / 1000).toString();
